...
export async function amendFilesForUnpkg(pkg: Package) {
  ...
  try {
    await exec({
      cwd: pkg.projectPath,
      cmd: `vite build --outDir dist/unpkg/prod`,
      env: {
        NODE_ENV: 'production',
  ...
  try {
    await exec({
      cwd: pkg.projectPath,
      cmd: `vite build --outDir dist/unpkg/prod-deprecated`,
      env: {
        NODE_ENV: 'production',
  ...
  try {
    await exec({
      cwd: pkg.projectPath,
      cmd: `vite build --outDir dist/unpkg/dev`,
      env: {
  ...
  try {
    await exec({
      cwd: pkg.projectPath,
      cmd: `vite build --outDir dist/unpkg/dev-deprecated`,
      env: {
        NODE_ENV: 'development',
        EMBER_DATA_FULL_COMPAT: 'false',
        IS_UNPKG_BUILD: 'true',
